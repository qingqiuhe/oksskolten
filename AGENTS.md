...
- Production deploys use the protected webhook flow: GitHub Actions publishes an image digest, then [`scripts/trigger-production-deploy.sh`](/Users/te/workspace/codex/oksskolten/scripts/trigger-production-deploy.sh) sends signed metadata to a server-side deploy endpoint.
- The public repository must not contain the production server address, login user, deploy directory, SSH key names, Cloudflare Tunnel token, MeiliSearch key, or webhook secret. Keep those in GitHub secrets or server-local config.
- The server-side entrypoint is [`scripts/production-deploy-agent.sh`](/Users/te/workspace/codex/oksskolten/scripts/production-deploy-agent.sh). It accepts only immutable `SERVER_IMAGE=ghcr.io/qingqiuhe/oksskolten@sha256:...` style image refs, updates only the `server` service, validates health, restarts `cloudflared`, and rolls back to the previous image on failure.
- Required GitHub Actions production variable: `PUBLIC_URL`.
- Required GitHub Actions production secrets: `PRODUCTION_DEPLOY_WEBHOOK_URL`, `PRODUCTION_DEPLOY_WEBHOOK_SECRET`.
- Server-local deploy config owns `DEPLOY_DIR`, `PROJECT_NAME`, `DATA_DIR`, `MEILI_MASTER_KEY`, `TUNNEL_TOKEN`, optional registry credentials, and `OKSSKOLTEN_DEPLOY_SECRET`.
- Do not treat a successful webhook call or local container start as sufficient proof of success. Always verify external `/api/health` and compare `buildDate`.
...
- `scripts/production-deploy-agent.sh` intentionally updates only `server` and only ensures `cloudflared` is running. Do not restart a healthy `cloudflared` from inside a webhook-triggered deploy; doing so severs the webhook response path. Do not widen it back to `compose up` on `meilisearch` / `rss-bridge` / `flaresolverr` during normal app deploys; restarting dependencies caused avoidable production instability during CD validation.
- When modifying `scripts/production-deploy-agent.sh`, preserve the rule that empty optional values must not overwrite remote `.env`. A deploy run without `MEILI_MASTER_KEY` must not blank the existing remote key.
- If production deploy appears stuck for a few minutes, check live public `/api/health` before assuming failure. The public endpoint may briefly return `502` while `server` and `cloudflared` rotate, then recover to the new digest before the workflow finishes.
