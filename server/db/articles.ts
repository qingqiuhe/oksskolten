...
export function getArticles(opts: {
  ...
  if (opts.collapseSimilar) {
    ...
    hasMore = shouldIncludeTotal ? (requestedLimit > 0 && pageRows.length > 0 && opts.offset + articles.length < total) : collapsed.hasMore
  } else {
    hasMore = shouldIncludeTotal ? (requestedLimit > 0 && pageRows.length > 0 && opts.offset + pageRows.length < total) : pageRows.length > requestedLimit
    articles = hasMore && !shouldIncludeTotal ? pageRows.slice(0, requestedLimit) : pageRows
  }
  if (!shouldIncludeTotal) {
