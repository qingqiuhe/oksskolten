...
function sortedChannelIds(ids: Iterable<number>): number[] {
...
function getFeedNotificationRuleRecordByRuleId(ruleId: number): FeedNotificationRuleRecord | null {
  ...
  return { ...rule, channel_ids: sortedChannelIds(rows.map(row => row.channel_id)) }
}
...
export function listNotificationChannelsByIds(ids: number[], userId?: number | null): NotificationChannel[] {
  ...
  return getDb().prepare(`
    ...
    WHERE id IN (${placeholders(ids.length)})
      AND ${scope.clause}
  `).all(...ids, ...scope.params) as NotificationChannel[]
}
...
export function createNotificationChannel(
  ...
  const scopedUserId = resolveUserId(userId)
  return getNamed<NotificationChannel>(`
...
export function updateNotificationChannel(
  ...
  if (fields.length === 1) return getNotificationChannelById(id, userId)

  const scopedUserId = resolveUserId(userId)
  if (scopedUserId == null) {
    return getNamed<NotificationChannel>(`UPDATE notification_channels SET ${fields.join(', ')} WHERE id = @id AND user_id IS NULL RETURNING *`, params)
  }
  return getNamed<NotificationChannel>(`UPDATE notification_channels SET ${fields.join(', ')} WHERE id = @id AND user_id = @user_id RETURNING *`, {
    ...params,
    user_id: scopedUserId,
  })
}
...
function deleteRuleChannelBindings(ruleId: number, channelIds: number[]): void {
  if (channelIds.length === 0) return
  getDb().prepare(`
    DELETE FROM feed_notification_rule_channels
    WHERE rule_id = ? AND channel_id IN (${placeholders(channelIds.length)})
  `).run(ruleId, ...channelIds)
}

function insertRuleChannelBindings(ruleId: number, channelIds: number[], initialLastArticleId: number | null): void {
  if (channelIds.length === 0) return
  const values = channelIds.map(() => '(?, ?, ?, NULL, NULL, datetime(\'now\'))').join(', ')
  const params = channelIds.flatMap(channelId => [ruleId, channelId, initialLastArticleId])
  getDb().prepare(`
...
export function upsertFeedNotificationRule(
  ...
  return getDb().transaction(() => {
    ...
    if (!rule) {
      const createdRule = getNamed<FeedNotificationRule>(`
      ...
      const updatedRule = getNamed<FeedNotificationRule>(`
    ...
    const existingIds = new Set(rule.channel_ids)

    const removedChannelIds = [...existingIds].filter(channelId => !desired.has(channelId))
    if (removedChannelIds.length > 0) {
      deleteRuleChannelBindings(rule.id, removedChannelIds)
    }

    const addedChannelIds = [...desired].filter(channelId => !existingIds.has(channelId))
    if (addedChannelIds.length > 0) {
      const initialLastArticleId = latestFeedArticleId(feedId)
      insertRuleChannelBindings(rule.id, addedChannelIds, initialLastArticleId)
    }

    return {
      ...rule,
      channel_ids: sortedChannelIds(desired),
    }
  })()
}
...
function mapNotificationTaskRows(rows: NotificationTaskRow[]): NotificationTaskRecord[] {
...
export function listNotificationTasks(userId?: number | null): NotificationTaskRecord[] {
  ...
  const rows = getDb().prepare(`
    ...
    ORDER BY
      ...
      lower(f.name),
      r.id
  `).all(...(scopedUserId == null ? [] : [scopedUserId])) as NotificationTaskRow[]

  return mapNotificationTaskRows(rows)
}
...
export function getNotificationTaskById(ruleId: number): NotificationTaskRecord | null {
  const row = getDb().prepare(`
    ...
    GROUP BY
      ...
      r.next_check_at,
      r.last_checked_at
  `).get(ruleId) as NotificationTaskRow | undefined

  return row ? mapNotificationTaskRows([row])[0] : null
}
...
export function updateNotificationTaskById(
  ...
  return getDb().transaction(() => {
    ...
    const nextContentMode = data.content_mode ?? existing.content_mode
    ...
    let nextChannelIds = existing.channel_ids
    if (data.channel_ids !== undefined) {
      const desired = new Set(data.channel_ids)
      const existingIds = new Set(existing.channel_ids)
      nextChannelIds = sortedChannelIds(desired)

      const removedChannelIds = existing.channel_ids.filter(channelId => !desired.has(channelId))
      if (removedChannelIds.length > 0) {
        deleteRuleChannelBindings(ruleId, removedChannelIds)
      }

      const addedChannelIds = [...desired].filter(channelId => !existingIds.has(channelId))
      if (addedChannelIds.length > 0) {
        const initialLastArticleId = latestFeedArticleId(existing.feed_id)
        insertRuleChannelBindings(ruleId, addedChannelIds, initialLastArticleId)
      }
    }
...
export function getPendingNotificationArticles(
  ...
  const articles = getDb().prepare(`
    ...
    LIMIT ?
  `).all(...params, maxArticles) as NotificationArticleRecord[]
